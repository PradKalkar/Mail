...
def compose(request):
    ...
    if request.method != "POST":
        return JsonResponse({"error": "POST request required.", "status": 400}, status=400) # Bad request

    # Check recipient emails
    data = json.loads(request.body) # deserialize request.body from JSON to python object
    ...
    # so that the email gets associated to every user involved
    users = set()
...
def email(request, email_id):
    ...
    try:
        email = Email.objects.get(user=request.user, pk=email_id) # here we are also specifying user so that one user cannot request for someone else's email
    ...
    # Update whether email is read or should be archived
        ...
        if data.get("read") is not None: # check if "read" is a key in data
